...
function formatWindow(crumb: DraftCrumb) {
  ...
  if (crumb.allDay) return format(start, "EEE, MMM d");
  ...
  return (
    <div className="relative">
      ...
      <div className="pointer-events-none absolute inset-0 flex items-center justify-between rounded-xl px-5">
        <motion.div style={{ opacity: leftHint }} className="flex items-center gap-1.5 text-orange-500">
          <Split className="h-5 w-5" />
          <span className="text-sm font-medium">Split</span>
        </motion.div>
        <motion.div style={{ opacity: rightHint }} className="flex items-center gap-1.5 text-emerald-600">
      ...
      <motion.div
        ...
        className="relative flex items-center justify-between gap-3 rounded-xl border border-black/10 bg-white p-4 shadow-sm active:cursor-grabbing dark:border-white/10 dark:bg-neutral-900"
      >
        <div className="min-w-0">
          {crumb.projectTitle && crumb.projectTitle !== crumb.title && (
            <div className="truncate text-xs font-medium text-indigo-500">{crumb.projectTitle}</div>
          )}
          <div className="truncate font-medium text-neutral-900 dark:text-neutral-100">{crumb.title}</div>
          <div className="text-xs text-neutral-500">{formatWindow(crumb)}</div>
          {crumb.notes && <div className="mt-1 text-xs text-neutral-400">{crumb.notes}</div>}
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-full border border-black/10 text-orange-500 disabled:opacity-30 dark:border-white/10"
          >
          ...
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-full bg-emerald-600 text-white disabled:opacity-30"
          >
